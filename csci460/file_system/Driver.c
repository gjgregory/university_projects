...
int DevWrite ( int BlockNumber,		// Logical block number
    ...
    if (fseek(partition, pos, SEEK_SET)) {
	   printf("DevWrite error: seek error at position %d\n", pos);
	   return 0;
   }

   // Write the data
   if (fwrite(Data, 1, BYTES_PER_SECTOR, partition) == 0) {
	   printf("DevWrite error: write error at position %d\n", pos);
	   return 0;
   }
   else
	   return 1;
...
int DevRead ( int BlockNumber,		// Logical block number
   ...
   // Seek to that position
   if (fseek(partition, pos, SEEK_SET)) {
	   printf("DevRead error: seek error at position %d\n", pos);
	   return 0;
   }

   // Read the data
   if (fread(Data, 1, BYTES_PER_SECTOR, partition) == 0) {
	   printf("DevRead error: read error at position %d\n", pos);
	   return 0;
   }
   else
	   return 1;
